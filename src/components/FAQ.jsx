...
function FAQ (){
    return(
        <section className="max-w-4xl text-center grid grid-cols-1 mx-auto py-8">
           ...
           <div>
            {FAQdata.map((item)=>(
                <div key={item.id} className="grid mt-5 p-4 max-h-12 max-w-4xl bg-slate-50 grid-cols-2 sm:grid-cols-3 mx-auto shadow-sm border border-slate-100 ">
                    <h3 className="">{item.question}</h3>
                    <button onClick={() => click(item.answer)} className=""><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
